...
object GenerateCaseDimensionTable {
  ...
  def apply(sparkSession: SparkSession, caseInputData: Map[String, DataFrame], trapDimension: DataFrame): (DataFrame, DataFrame) = {
    val windowSpec = Window.orderBy("answer_id")
    val dimCaseDf : DataFrame = caseInputData("task_on_geo_object").as("togo").join(caseInputData("traps").as("go"), "gid")
      ...
      .where(col("question_id").isin(
        ...
        when(col("question_id").isin("BMSB.INST.Q2", "BMSB.INSTNEW.Q3"), lit("Hedges and borders"))
          ...
          .otherwise(col("question_id")).alias("category"),
        when(col("answer_id").isin("BMSB.INST.Q2.A9", "BMSB.INSTNEW.Q3.A9"), lit("Altra siepe o bordura"))
          ...
          .otherwise(col("a.text")).alias("environmental_element (manual)")
      ).distinct()
      .withColumn("cid", row_number().over(windowSpec))
    ...
    val bridgeTrapCaseDf = caseInputData("task_on_geo_object").as("togo").join(caseInputData("given_answer").as("ga"), "togo_id")
      ...
      .where(col("question_id").isin(
        ...
        "BMSB.INST.Q10", "BMSB.INSTNEW.Q11", "BMSB.INST.Q12", "BMSB.INSTNEW.Q13",
        "BMSB.INST.Q14", "BMSB.INSTNEW.Q15"
      )).select(col("gid"), col("answer_id"), col("ga.text")).join(dimCaseDf.as("t"), "answer_id")
      .where((col("text").isNotNull && col("text") === col("environmental_element (manual)")) || col("text").isNull)
      .select(col("gid"), col("cid"))
    //add the traps that are not linked to the bridge table
    val (newDimensionTable, newBridgeTable) = DimensionsTableUtils.addNotNearRows(sparkSession, "cid", dimCaseDf.drop("answer_id"), bridgeTrapCaseDf, trapDimension)
